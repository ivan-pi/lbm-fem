...
int main(int argc, char **argv) {
  ...
  for (int a = 0; a < 9; ++a) for (int i = 0; i < N * N; ++i) f[a][i] = 1.0 + 1e-3 * ((i * 7919) % 13);
  ...
  for (int s = 0; s < steps; ++s)
    for (int a = 1; a < 9; ++a) {
      ...
      const double *F = f[a].data(); double *R = r[a].data();
      for (int j = 0; j < N; ++j) {
        const int jm = (j + N - 1) % N * N, j0 = j * N, jp = (j + 1) % N * N;
        for (int i = 0; i < N; ++i) {
          const int im = (i + N - 1) % N, ip = (i + 1) % N;
          R[j0 + i] = w[0][0] * F[jm + im] + w[1][0] * F[jm + i] + w[2][0] * F[jm + ip]
                    + w[0][1] * F[j0 + im] + w[1][1] * F[j0 + i] + w[2][1] * F[j0 + ip]
                    + w[0][2] * F[jp + im] + w[1][2] * F[jp + i] + w[2][2] * F[jp + ip];
        }
      }
    }
  const double sec = std::chrono::duration<double>(std::chrono::steady_clock::now() - t0).count();
  double chk = 0; for (int a = 1; a < 9; ++a) chk += r[a][N + 1];
  printf("N=%d: %.3f s for %d steps, %.3f ms/step, %.1f MNUPS, %.2f GB/s (16 vector passes/step), %.2f GFlop/s (17 flops per node and direction), %.0f ns/cell/direction (chk %g)\n",
         N, sec, steps, 1e3 * sec / steps, 1e-6 * N * N * steps / sec, 16.0 * 8 * N * N * steps / sec / 1e9, 17.0 * 8 * N * N * steps / sec / 1e9, 1e9 * sec / (steps * 8.0 * N * N), chk);
}
