...
| | |
|---|---|
| `include/lbfem/` | the header-only library: lattice, discretization, collision, advection, mass solves, streaming, schemes, test cases (see [Code structure](#code-structure)) |
...
| `analysis/` | sympy/numpy analysis of the schemes: element matrices and stencils (`stencils.py`), von Neumann analyses (`*_spectral.py`), TikZ figure generator (`make_tikz.py`) |
| `benchmarks/` | stand-alone micro-benchmarks: empirical roofline (`roofline.cc`), stencil "speed of light" (`stencil_bench.cc`) |
| `tools/` | `elem.cc`: element matrices from deal.II `FEValues`, to check against `analysis/stencils.py`; `mass_preconditioners.cc`: spectra and CG convergence of the mass solves; `cg_fused_hooks.cc`: reproducer of the fused-CG preconditioner slowdown in deal.II ([docs/dealii-fused-cg.md](docs/dealii-fused-cg.md)) |
| `data/cavity/` | lid-driven cavity results ($`Re`$ = 400, 1000, 3200, 5000; Q2, $`32^2`$ elements) |
| `docs/figures/` | stencil figures (standalone TikZ, generated by `analysis/make_tikz.py`) |
...
## Build and run
    ...
    gnuplot ../scripts/plot_vortex.gp                     # -> vortex.png
...
(`analysis/lbm_spectral2.py`) gives a critical per-axis Courant number of 1.00 for
...
1-D Galerkin symbols on a uniform grid (`analysis/`, same analysis as above):
