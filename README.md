...
| | |
|---|---|
| `src/` | the solver, `cgdbe.cc` (deal.II, matrix-free) |
...
| `tools/` | `elem.cc`: element matrices from deal.II `FEValues`, to check against `analysis/stencils.py` |
...
`src/cgdbe.cc` is one file: the D2Q9 lattice, the parameters, the three test cases
