...
struct Identity // never applied: only the type matters for IterationWorker
{
  ...
  vmult(VT &,
        ...
        const std::function<void(unsigned, unsigned)> &) const
  {}
};
struct RangeOnly
{
  ...
  void
  apply_to_subrange(unsigned b, unsigned e, const double *s, double *d) const
  {
    D.apply_to_subrange(b, e, s, d);
  }
};

template <typename P>
void
run(const char *name, const P &prec, const unsigned n)
{
  VT                                     x(n), b(n);
  std::mt19937                           g(1);
  ...
  w.alpha          = 0.3;
  w.beta           = 0.2;
  w.previous_alpha = 0.25;
  w.previous_beta  = 0.15;
  for (auto &v : w.v)
    v = u(g);
  ...
  const double                           t_before_odd  = time([&] {
    for (unsigned s = 0; s < n; s += chunk)
      w.operation_before_loop(3, s, std::min(n, s + chunk));
  });
  const double                           t_before_even = time([&] {
    for (unsigned s = 0; s < n; s += chunk)
      w.operation_before_loop(2, s, std::min(n, s + chunk));
  });
  const double                           t_after       = time([&] {
    for (unsigned s = 0; s < n; s += chunk)
      w.operation_after_loop(s, std::min(n, s + chunk), sums);
  });
...
main(int argc, char **argv)
{
  ...
  std::printf("deal.II %s, VectorizedArray<double> width %u\n",
              ...
              static_cast<unsigned int>(VectorizedArray<double>::size()));
