...
static double now() { return std::chrono::duration<double>(clk::now().time_since_epoch()).count(); }

__attribute__((noinline)) double fma_peak(long iters, double x) {
  ...
  for (long i = 0; i < iters; ++i) {
    a0 = a0 * y + x; a1 = a1 * y + x; a2 = a2 * y + x; a3 = a3 * y + x;
    a4 = a4 * y + x; a5 = a5 * y + x; a6 = a6 * y + x; a7 = a7 * y + x;
  }
...
int main() {
  {
    ...
    double t = now(); double r = fma_peak(it, 1e-9); t = now() - t;
  ...
  for (long n : {66049L, 4'000'000L}) {
    ...
    double *F[Q], *G[Q];
    for (int a = 0; a < Q; ++a) { F[a] = f[a].data(); G[a] = g[a].data(); }
    ...
    for (int r = 0; r < reps; ++r) {
      double t = now();
      for (int a = 0; a < Q; ++a) for (long i = 0; i < n; ++i) G[a][i] = 1.0001 * F[a][i] + 1e-3;
      best = std::min(best, now() - t);
    }
    printf("n=%-8ld (%5.1f MB/array) stream 9x(in->out):          %6.2f GB/s  %5.2f GFlop/s\n",
           n, MB, 18.0 * n * 8 / best / 1e9, 2.0 * Q * n / best / 1e9);
